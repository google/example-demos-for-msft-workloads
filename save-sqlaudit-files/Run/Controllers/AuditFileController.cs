...
namespace AspNetCoreWebApi6.Controllers
{
    ...
    public class AuditFileController : ControllerBase
    {
        ...
        public AuditFileController(
            ...
            ILogger<AuditFileController> logger
        )
        {
        ...
        public async Task<IActionResult> Post([FromBody] JsonElement body)
        {
            string settingsFileName = (
                Environment.GetEnvironmentVariable("SETTINGS_FILENAME") ?? "mySettings.json"
            );
            string topicId = Environment.GetEnvironmentVariable("TOPIC_ID") ?? "";
            bool publishToPubSub = bool.Parse(
                Environment.GetEnvironmentVariable("DO_PUBSUB_PUBLISH") ?? "false"
            );
            bool saveParquetToGCS = bool.Parse(
                Environment.GetEnvironmentVariable("DO_PARQUET_GCS_SAVE") ?? "false"
            );
            string saveParquetBucketName =
                Environment.GetEnvironmentVariable("PARQUET_GCS_BUCKETNAME") ?? "";
            bool saveToLog = bool.Parse(
                Environment.GetEnvironmentVariable("DO_LOG_SAVE") ?? "false"
            );
            ...
            {
                var errMessage =
                    "400 Bad Request. DO_PUBSUB_PUBLISH, DO_PARQUET_GCS_SAVE and DO_LOG_SAVE cannot be false at the same time.";
                var result = new BadRequestObjectResult(
                    new { message = errMessage, currentDate = DateTime.Now }
                );
            ...
            {
                var errMessage =
                    "400 Bad Request. The name element in the post request cannot be empty.";
                var result = new BadRequestObjectResult(
                    new { message = errMessage, currentDate = DateTime.Now }
                );
                _logger.LogInformation(errMessage);
            ...
            {
                var errMessage =
                    "400 Bad Request. The bucket element in the post request cannot be empty.";
                var result = new BadRequestObjectResult(
                    new
                    {
                        message = "400 Bad Request. The bucket element in the post request cannot be empty.",
            ...
            if (saveParquetToGCS)
            {
                ...
                _logger.LogInformation(
                    "Uploaded parquet file {fileName} with {count} entries to {bucket} ",
                    parquetObjectName,
                    SQLAuditEventData.Count,
                    saveParquetBucketName
                );
