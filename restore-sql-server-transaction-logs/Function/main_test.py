...
def test_get_environment_variables(mock_logger):
    with patch('main.os.environ', {'USE_FIXED_FILE_NAME_FORMAT': 'True', 'PROCESSED_BUCKET_NAME': 'bucket', 'MAX_OPERATION_FETCH_TIME_SECONDS': '60'}):
        result = get_environment_variables(mock_logger)
        assert result == ('bucket', 'True', 60)


def test_extract_info_from_file_name():
    result = extract_info_from_file_name('csql-instance-name_database-name_full_backup.bak')
    assert result == ('csql-instance-name', 'database-name', 'FULL', 'true')

    with pytest.raises(RuntimeError):
        extract_info_from_file_name('invalid-filename.bak')


def test_extract_info_from_file_metadata(mock_storage_client, mock_source_bucket):
    # Test case when metadata is correctly defined
    mock_blob = MagicMock()
    mock_blob.metadata = {'CloudSqlInstance': 'instance', 'DatabaseName': 'db', 'BackupType': 'FULL', 'Recovery': 'false'}
    mock_source_bucket.get_blob.return_value = mock_blob
    mock_storage_client.bucket.return_value = mock_source_bucket

    result = extract_info_from_file_metadata(mock_storage_client, 'bucket', 'object_name')
    assert result == ('instance', 'db', 'FULL', 'true')

    # Test case when metadata is missing
    mock_blob.metadata = {}
    with pytest.raises(RuntimeError):
        extract_info_from_file_metadata(mock_storage_client, 'bucket', 'object_name')


def test_get_import_context():
    result = get_import_context('source_bucket', 'object_name', 'db', 'FULL', 'true')
    expected_result = {
        "importContext": {
            "fileType": "BAK",
...
def test_delete_processed_blob(mock_source_bucket, mock_logger):
    ...
    mock_logger.info.assert_called_with("Deleted object object_name from the source bucket.")


def test_handle_error(mock_source_bucket, mock_logger):
    error_response = {"errors": "Msg 4326"}
    handle_error(error_response, mock_source_bucket, 'object_name', mock_logger)
    mock_source_bucket.delete_blob.assert_called_with('object_name')
...
def test_copy_blob_to_processed_bucket(mock_storage_client, mock_source_bucket):
    mock_processed_bucket = MagicMock()
    mock_storage_client.bucket.side_effect = [mock_source_bucket, mock_processed_bucket]
    copy_blob_to_processed_bucket('processed_bucket', mock_storage_client, mock_source_bucket, 'object_name')
...
def test_fn_restore_log(
        mock_auth_default, mock_build, mock_sleep, mock_copy_blob, mock_delete_blob, mock_get_import_context,
        mock_extract_info, mock_env_variables, mock_error_reporting, mock_logging,
    ...
    mock_extract_info.return_value = ('instance-name', 'database-name', 'FULL', "true")
    cloud_event_object = CloudEvent(attributes={"type": "com.example.sampletype1","source": "https://example.com/event-producer",}, 
                                    data={'bucket': 'source_bucket', 'name': 'instance-name_database-name_somemoreinfo.bak'})
    cloud_event_folder = CloudEvent(attributes={"type": "com.example.sampletype1","source": "https://example.com/event-producer",},
                                    data={'bucket': 'source_bucket', 'name': 'foldername/'})

    # Test case for successful import
    mock_operation_response_done = {'name': 'operation_name', 'status': 'DONE'}
    mock_operation_response_done_error = {'name': 'operation_name', 'status': 'ERROR', 'error': 'some_error'}
    mock_service = mock_build.return_value
    mock_service.operations.return_value.get.return_value.execute.return_value = mock_operation_response_done
    result = fn_restore_log(cloud_event_object)
    assert result == ('Operation succeded.', 200)

    # Test case for failed import
    mock_service.operations.return_value.get.return_value.execute.return_value = mock_operation_response_done_error
    with pytest.raises(RuntimeError):
